...
class CreateAuftragPositionTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        if (!app()->environment('production')) {
            Schema::create('auftrag_position', function (Blueprint $table) {
                $table->id();
                $table->integer('auftrag');
                ...
                $table->integer('geliefert');
                $table->decimal('geliefert_menge', 14, 4);
                $table->integer('explodiert');
                $table->integer('explodiert_parent')->index('explodiert_parent');
                $table->timestamp('logdatei')->useCurrent()->useCurrentOnUpdate();
                $table->decimal('rabatt', 10, 2);
                $table->integer('nachbestelltexternereinkauf')->nullable();
                ...
                $table->date('potentiellerliefertermin')->nullable();
                ...
                $table->index(['auftrag', 'artikel'], 'auftrag');
            });
        }
    }
    ...
    {
        if (!app()->environment('production')) {
            Schema::dropIfExists('auftrag_position');
        }
    }
}
